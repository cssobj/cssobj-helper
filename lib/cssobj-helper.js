...
export function syntaxSplit (str, splitter, pair, test, final) {
  var isFeature, isSplitter
  for (var c, i = 0, len=str.length, ast = 0, prev = 0, feature = [], segment = [], result = []; c = str.charAt(i) || i==len; i++) {
    if (!ast && c in pair) ast = c
    else if (ast && c == pair[ast]) ast = 0
    if (ast) {
      segment.push(c)
    } else {
...
// console.log(syntaxSplit('as&fdf,&&df[d&s"oi&sd,jf"f,i[&df,fij]jo]&, oij&', ',', pair, c => '&&&'.indexOf(c) > -1, f => ('----')))
